...
int update(unsigned char **p, size_t *l, void *ctx, size_t ctx_len)
{
	...
	if ((buffer[0] == 0x00)&&(buffer[1] == TFTP_OPCODE_READ)) {
		...
		if (terminal->object->module_data_size != sizeof(BLOB_RECORD)) {
			api_log_printf("[AK306] Invalid config record, terminal_id=%d\r\n", terminal->id);
			*l = 0;
			return 0;
		}
		...
		api_log_printf("[AK306] Terminal authorized for update, terminal_id=%u\r\n", terminal->id);
		...
		if (memcmp(config->fw, "default", 7)) {
			...
			char fw[64];
			sprintf(fw, "fw%04u.306", config->actual_fw_ver);
			file_name += fw;
		}
		...
		if (terminal->f) {
			...
			api_log_printf("[AK306] Send frame 1, size %u, terminal_id=%u\r\n", terminal->tx_cmd.header.size, terminal->id);
			...
			*l = terminal->tx_cmd.header.size;
			...
			api_log_printf("[AK306] Unable to open file [%s], terminal_id=%u\r\n", file_name.c_str(), terminal->id);
	...
	if ((buffer[0] == 0x00)&&(buffer[1] == TFTP_OPCODE_ACK)&&(*l >= 4)) {
		...
		if (terminal->tx_cmd.header.seqNo == buffer[3]) {
			...
			api_log_printf("[AK306] Frame acked, terminal_id=%u\r\n", terminal->id);

			if ((terminal->tx_cmd.header.size < 512)||(terminal->tx_cmd.header.seqNo == 88)) {

				api_log_printf("[AK306] Transfer done, terminal_id=%u\r\n", terminal->id);
