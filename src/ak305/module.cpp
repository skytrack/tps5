...
static const char *name				= "AK305 module, v1.0";

static const char *terminal_name	= "AK305";
static const int   terminal_type	= OBJECT_TYPE;

static std::string host				= "*";
static std::string dport			= "45591";
static std::string uport			= "45592";
...
int set_var(int var_type, void *p)
{
	...
	case MODULE_VAR_OPTION:
		...
		if (strcmp(op->name, "fw_dir") == 0) {
			...
			api_log_printf("[AK305] Warning: unknown config option '%s'\r\n", op->name);
		}
		break;
...
static const SESSION_HANDLERS update_handlers = { 
	(SESSION_OPEN)update_session_open, 
	(SESSION_CLOSE)update_session_close, 
...
int start()
{
	...
	api_listen_tcp(api_tag, host.c_str(), dport.c_str(), &data_handlers);
	api_listen_tcp(api_tag, host.c_str(), uport.c_str(), &update_handlers);

	now = time(NULL);

	api_log_printf("[AK305] Started\r\n");
