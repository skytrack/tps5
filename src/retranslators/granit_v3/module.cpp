...
static const char *name					= "granit v3";
static const unsigned int protocol_id	= 1;
...
RETRANSLATOR *create_retranslator(const char *device_id, const char *host, unsigned short port, const char *login, const char *password)
{
	...
	pRetranslator->nupe			= strtoul(device_id, NULL, 10);

	spinlock_init(&pRetranslator->spinlock);

	api_log_printf("[GRANIT V3] Create retranslator for '%s'\r\n", device_id);
...
{
	api_log_printf("[GRANIT V3] Destroy retranslator for '%s' with socket #%d\r\n", pRetranslator->device_id.c_str(), pRetranslator->sock);
...
int start()
{
	...
	api_log_printf("[GRANIT V3] Started\r\n");
...
int stop()
{
	...
	api_log_printf("[GRANIT V3] Stopped\r\n");
